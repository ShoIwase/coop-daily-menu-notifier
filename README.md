...
`.github/workflows/daily_notify.yml` で **毎週月曜 17:37 JST**（`37 8 * * 1` UTC）に自動実行。毎時00分は GitHub Actions の schedule が最も混雑し数時間〜半日単位で遅延するため（実際に2026-08-31は7時間半遅れて翌01:26 JSTに実行された）、ピークを外した半端な分に置いている。それでも schedule は遅延しうる仕様なので、到着時刻は多少前後する。`workflow_dispatch` で手動実行も可能（`debug_scrape` 入力をtrueにするとデバッグ用スクリーンショット/HTMLをアーティファクトとして保存する）。手動実行時もキャッシュの読み込み・更新・コミットは通常実行と同様に行われる点に注意。
