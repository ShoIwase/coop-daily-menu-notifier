...
4. AWS Bedrock（Claude Haiku, `jp.anthropic.claude-haiku-4-5-20251001-v1:0`, ap-northeast-1）のビジョン機能で画像を読み取り、「月: 主菜（副菜...）」の曜日別リストに月〜金5日分すべてを整形
   - 各コースの表組みの直前には、別枠の「今週のおすすめ」バナー（そのコース自体のメニューではない）が隣接しており、クロップ範囲から完全には除外できない。誤読を防ぐため、プロンプトでこのバナーを無視して品番（例: 902）付きの囲み枠から読み取るよう明示している
