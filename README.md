...
`.github/workflows/daily_notify.yml` で **毎週月曜 12:17 JST**（`17 3 * * 1` UTC）に自動実行。毎時00分は GitHub Actions の schedule が最も混雑し数時間〜半日単位で遅延するため（実際に2026-08-31は7時間半遅れて翌01:26 JSTに実行された）、ピークを外した半端な分に置いている。それでも schedule は遅延しうる仕様なので、到着時刻は多少前後する。なお来週分PDFへの切り替わりは月曜0時ちょうどなので、月曜内であれば実行時刻を動かしてもキャッシュされる内容は変わらない。`workflow_dispatch` で手動実行も可能（`debug_scrape` 入力をtrueにするとデバッグ用スクリーンショット/HTMLをアーティファクトとして保存する）。手動実行時もキャッシュの読み込み・更新・コミットは通常実行と同様に行われる点に注意。
...
- 献立カレンダーPDFのURL（日付フォルダ）が毎週どういう規則で決まるかは未解明。そのため固定URLを推測するのではなく、毎回ログインして実際のリンクを取得する方式にしている。
- ログイン後ページに掲載されているPDFは常に「実行日を含む週の“次の週”」分のみで、切り替わりは各週月曜0時ちょうど（配達のない週はスキップ）。つまり実行タイミングをどうずらしても、その場で取得する限りその週自身の献立には辿り着けない。これが原因で以前は通知内容が実際の配達と1週ズレる不具合があったため、「1週間前倒しキャッシュ」方式（上記6番）で対処している。`data/menu-cache.json` を初期化した状態でリポジトリをforkする場合、初回実行は送信スキップになり配信が1回分空くので、必要なら手動でキャッシュを仕込むこと。
